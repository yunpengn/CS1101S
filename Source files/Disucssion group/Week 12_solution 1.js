...
// Discussion Group Week 12 - solution 1
...
/*
*******************************************************************************
Q4 Functions to handle while loops
*******************************************************************************
*/
...
function evaluate_while_statement(stmt, env) {
    while (is_true(evaluate(while_predicate(stmt),env))) {
        evaluate(while_statements(stmt), env);
    }
    return undefined;
}
...
/*
*******************************************************************************
Q3 Functions to handle boolean operations
*******************************************************************************
*/
function is_boolean_operation (stmt) {
    return is_tagged_object(stmt, "boolean_op");
}
function evaluate_boolean_operation (op, env, operands) {
    ...
    if (op === "&amp;&amp;") {
        ...
        error("Unrecognised boolean operator " + op);
    }
}
...
/*
*******************************************************************************
Q2 Functions to handle assignment
*******************************************************************************
*/
function set_variable_value(variable, value, env) {
    var frame = lookup_variable_frame(variable, env);
    add_binding_to_frame(variable, value, frame);
}

function is_assignment_statement(stmt) {
...
function evaluate_assignment(stmt, env) {
    var left = assignment_variable(stmt);
    var right = evaluate(assignment_value(stmt), env);

    set_variable_value(left, right, env);
}
/*
*******************************************************************************
    ...
    if (is_self_evaluating(stmt)) {
        ...
        return make_return_value(
            evaluate(return_statement_expression(stmt),
                     env));
    } else if (is_assignment_statement(stmt)) { //Q2
        return evaluate_assignment(stmt,env);
    } else if (is_boolean_operation(stmt)) { //Q3
        return evaluate_boolean_operation(operator(stmt), env, operands(stmt));
    } else if (is_while_statement(stmt)) { //Q4
        return evaluate_while_statement(stmt,env);
    } else {
...
function primitive_identifier_names () {
   return map(head, primitive_identifiers);
}

function primitive_identifier_values () {
   return map(tail, primitive_identifiers);
}

function setup_environment() {

    var environment_with_global_objects = extend_environment(primitive_identifier_names(),
                                                             primitive_identifier_values(),
                                                             the_empty_environment);

    var initial_env = extend_environment(primitive_function_names(),
                                         primitive_function_objects(),
                                         environment_with_global_objects);
    return initial_env;
}
// Task 1
