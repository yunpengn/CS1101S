...
var make_signed_message = pair;
var get_message = head;
var get_signature = tail;

function encrypt_and_sign(msg,key1,key2) {
    var emsg = RSA_encrypt(msg,key2);
    return make_signed_message(emsg,
                               bigInt_to_int(RSA_transform(compress(emsg),key1)));
}

function authenticate_and_decrypt(signed_message,public_key1,private_key2) {
    var message = get_message(signed_message);
    var signature = get_signature(signed_message);
    var decrypted_message = RSA_decrypt(message, private_key2);
    var unencrypted_signature = compress(message);

    if (equalsBigInt(unencrypted_signature,
                     RSA_transform(int_to_bigInt(signature), public_key1)) === 1)
        return decrypted_message;
    else
        console.log("WARNING: Signature does not match sender's identity.");
}
...
function solve_ax_plus_by_eq_1(a,b) {
    if (b === 0 && a === 1) {
        ...
        var p = solve_ax_plus_by_eq_1(b,
                                      bigInt_to_int(modulo(int_to_bigInt(a),
                                                           int_to_bigInt(b))));

        if (is_pair(p)) {
            return pair(tail(p),
                        head(p) - (tail(p) * Math.floor(a/b)));
        } else {
            return p;
        }
    }
...
function crack_rsa(key) {
    ...
    var p = bigInt_to_int(smallest_divisor(int_to_bigInt(n)));
    var q = n / p;
    return make_key(n,
                    bigInt_to_int(invert_modulo(int_to_bigInt(e),
                                             mult(int_to_bigInt((p-1)),
                                                  int_to_bigInt((q-1))))));
}
...
export_symbol('encrypt_and_sign', encrypt_and_sign);
export_symbol('authenticate_and_decrypt', authenticate_and_decrypt);
export_symbol('solve_ax_plus_by_eq_1', solve_ax_plus_by_eq_1);
export_symbol('crack_rsa', crack_rsa);
export_symbol('is_string', is_string);
export_symbol('timed', timed);
})(window);
