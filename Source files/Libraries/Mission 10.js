...
function make_RSA_key_pair_generator(solver) {
	return function() {
	    ...
	    if (equalsBigInt(p,q) === 1) { // check that we haven't chosen the same prime twice
	        return make_RSA_key_pair_generator(solver);
	    } else {
	        var n = mult(p,q);
	        var m = mult(addInt(p, -1), addInt(q, -1));
	        var e = select_exponent(m);
	        var d = invert_modulo(solver,e,m);

	        // In case either p or q (or both) is not prime
	        if (is_string(d) && d === "gcd not 1") {
	            return make_RSA_key_pair_generator(solver);
	        } else {
				return make_key_pair(make_key(bigInt_to_int(n),
                                              bigInt_to_int(e)),
                                     make_key(bigInt_to_int(n),
                                              bigInt_to_int(d)));
	        }
	    }
	};
...
function invert_modulo(solver,e,m) {
	if (bigInt_GCD_is_1(e,m)) {
		var solved_tail = tail(solver(bigInt_to_int(m),bigInt_to_int(e)));
...
export_symbol('make_RSA_key_pair_generator', make_RSA_key_pair_generator);
